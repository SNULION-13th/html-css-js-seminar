...
likeCountNum = likeCount.innerText; // #like-count 태그 안의 텍스트를 가져오기

console.log(likeCountNum); // 1003
console.log(likeCountNum + 1); // 10031 : 예상치 못한 결과!!
console.log(typeof likeCount); // object

toInt = parseInt(likeCountNum); // int로 형변환!
...
const alarm = document.querySelector("#alarm_img");
...
blackHeart.addEventListener("click", () => {
  ...
  //알림 보내주기
  alarm.style.right = "0px";
  setTimeout(() => {
    alarm.style.right = "-300px";
  }, 1200);
});
...
const icon_header = document.querySelector(".icon-list");
const icon_article = document.querySelectorAll(".feed-buttons");

icon_header.addEventListener("mouseover", () => {
  icon_header.style.opacity = "0.5";
});
icon_header.addEventListener("mouseleave", () => {
  icon_header.style.opacity = "1";
});

icon_article.forEach((elem) => {
...
//const commentInput = document.querySelector(".comment");
const commentForm = document.querySelector(".comments-create-form");
let emoji_open = false;

// Toggle emoji modal when smile icon is clicked
...
// Prevent form submission when clicking emoji button
